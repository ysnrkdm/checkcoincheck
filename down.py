...
def download_files():
    ...
    br.form = br.forms()[0]
    br['email'] = "vitasse0101@gmail.com"
    br['password'] = "indigo1nd1g0"
    response = br.submit()
    # print(br.title())
    ...
    res = br.retrieve('https://coincheck.com/sells/download.csv', 'sells.csv')
    res = br.retrieve('https://coincheck.com/buys/download.csv', 'buys.csv')
    res = br.retrieve('https://coincheck.com/send/download.csv', 'send.csv')
    res = br.retrieve('https://coincheck.com/deposit_bitcoin/download.csv', 'deposit_bitcoin.csv')
    ...
    res = br.retrieve(orders_url + orders_url_start_date + orders_url_end_date, 'orders.csv')

    print('5 files download.')


def fetch_a_file_and_store_db(filename):
    df = pd.read_csv(filename)
    dbname = COINCHECK_DB
    tablename = q = re.findall('(.*?)\.csv', filename)[0]
